#Simulation of phenotypes controlled by averagely tagged causal variants. Phenotypic heritability was 0.8 and QTL number was 100
library("data.table")
...
geno = as.data.frame(fread("aseq.raw"))
...
cleang<-clean
cleang1<-cleang[,-1]
#Read "weights.all" file, "weights.all" is calculated using the --calc-weights-all of LDAK software, where the column named "Eff_Tagging" is the tagging condition of each markers
weights<-as.data.frame(fread("sections1000/weights.all"))
...
#Simulation of phenotypes controlled by averagely tagged causal variants, 100 QTL were randomly selected on the genome
locinum<-sample(data2[,1],100,replace=FALSE)
